...
export TELEGRAPH_ACCESS_TOKEN="…"   # 可选：在 Telegram 中用 Telegraph 链接承载全文；不设则内联前 3 条明细
./scripts/steam_key_daily.sh --state-path <STATE_JSON> --send --write-state --chat-id 529436356
```

**顺序（脚本已遵守）：** 读 `--state-path` → 抓 RSS 与 ITAD 详情 → 对 **新增/变更** 捆绑包请求 ITAD `games/historylow/v1`（`country=CN`，国区史低 CNY）→ 仅当存在 **值得买** 的新增或变更时：若已配置 `TELEGRAPH_ACCESS_TOKEN`（默认开启），则对每个捆绑包调用 [Telegraph API](https://telegra.ph/api) `createPage` 上传 Tier 明细，再用 `./scripts/telegram.sh send` 发送**短摘要 + 各页链接**；否则回退为旧行为（仅摘要中内联前 3 条的 Tier 全文）。最后写回 state（若带 `--write-state`）。
...
- 生成推送摘要：默认经 Telegraph 每包一页（Tier 下列出全部游戏）；无 token 时摘要内仅前 3 条内联全文
...
- 环境变量：`ITAD_API_KEY`（或 `ISTHEREANYDEAL_API_KEY` / `APIKEY`）用于史低与价值过滤；`TELEGRAPH_ACCESS_TOKEN` 来自 [@Telegraph](https://t.me/telegraph) 机器人。关闭上传：`--no-telegraph`。
