...
**顺序（脚本已遵守）：** 读 `--state-path` → 抓 RSS 与 ITAD 详情 → 对 **新增/变更** 捆绑包请求 ITAD `games/historylow/v1`（`country=CN`，国区史低 CNY）→ 仅当存在 **值得买** 的新增或变更且未 `--no-telegraph` 时：按顺序取 **Telegraph token**（`TELEGRAPH_ACCESS_TOKEN` → 配置文件 → 若本次会真实发送且非 `--dry-run`，则调用 [createAccount](https://telegra.ph/api) 自动建号并写入 `tools/steam_key_daily/telegraph_account.json`）→ 对每个捆绑包 `createPage` 上传 Tier 明细 → `./scripts/telegram.sh send` 发**短摘要 + 各页链接**。无可用 token 或上传全失败时回退为摘要内联前 3 条 Tier 全文。最后写回 state（若带 `--write-state`）。

**配置文件：** 默认路径 `tools/steam_key_daily/telegraph_account.json`（已在 `.gitignore`，勿提交）。可用环境变量 `TELEGRAPH_CONFIG_PATH` 或 `--telegraph-config <path>` 覆盖。字段示例见同目录 `telegraph_account.example.json`。
