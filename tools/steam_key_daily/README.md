...
# TELEGRAPH_ACCESS_TOKEN / TELEGRAPH_CONFIG_PATH 均可选；不设则首次真实推送时自动 createAccount 并写入配置文件
./scripts/steam_key_daily.sh --state-path <STATE_JSON> --send --write-state --chat-id 529436356
```

**顺序（脚本已遵守）：** 读 `--state-path` → 抓 RSS 与 ITAD 详情 → 仅对 **新增** 捆绑包请求 ITAD `games/historylow/v1`（`country=CN`，国区史低 CNY）→ 仅当存在 **值得买** 的新增且未 `--no-telegraph` 时：按顺序取 **Telegraph token**（`TELEGRAPH_ACCESS_TOKEN` → 配置文件 → 若本次会真实发送且非 `--dry-run`，则调用 [createAccount](https://telegra.ph/api) 自动建号并写入配置文件）→ 对每个捆绑包 `createPage` 上传 Tier 明细 → `./scripts/telegram.sh send` 发**短摘要 + 各页链接**。无可用 token 或上传全失败时回退为摘要内联前 3 条 Tier 全文。最后写回 state（若带 `--write-state`）。

**配置文件：** 默认与 **`--state-path` 同目录**下的 `telegraph_account.json`（与记忆 state 一起持久化，便于 Cursor 自动化挂载同一卷）。可用 `TELEGRAPH_CONFIG_PATH` 或 `--telegraph-config <path>` 覆盖。字段示例见 `tools/steam_key_daily/telegraph_account.example.json`。若路径落在本仓库内且含密钥，请勿提交该文件。

**「值得买」规则（默认）：** 至少一个 Tier 的档位价（CNY）≤ 该 Tier 内各 Steam 游戏 **国区史低之和** × `--min-savings-ratio`（默认 `0.95`）。缺 Key、API 失败或某款史低缺失时，该次新增 **不推送**（`runtime.json` 中可见原因）。关闭过滤：`--no-value-filter`。
...
- 仅对 **added** 调用 ITAD History Low（CN / CNY），摘要中展示每款史低
- 按国区史低总和过滤「值得买」档位后再推送
- diff 会忽略默认 **0.5 CNY 以内** 的价格微幅波动；同一 Fanatical 捆绑包在 `/en/...` 与 `/...` 间切换时也会视为同一条记录，避免重复推送
- 增量对比（默认全量历史；首次空记忆可限制最近 N 条）
- 生成推送摘要：默认经 Telegraph 每包一页（Tier 下列出全部游戏）；无 token（且未触发自动建号）或上传全失败时摘要内仅前 3 条内联全文
...
2) 巡检并发送（仅 **值得买** 的新增才发；需 `ITAD_API_KEY`）：
...
- `diff.json`: added/changed/removed 详情（其中 `changed` 固定为空，不再参与日报推送）
- `summary.txt`: 可直接发送的摘要文本（为空表示无新增）
...
- 环境变量：`ITAD_API_KEY`（或 `ISTHEREANYDEAL_API_KEY` / `APIKEY`）用于史低与价值过滤。Telegraph：`TELEGRAPH_ACCESS_TOKEN` 可显式覆盖；否则读配置文件；首次真实推送时脚本可自动 `createAccount` 写入配置（`--dry-run` 不会建号/上传）。关闭上传：`--no-telegraph`。
