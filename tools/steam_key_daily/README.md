...
**记忆 JSON 键名：** `steam_key_daily_state_v1`；结构与脚本写入的 state 一致（`last_run_at` + `bundles[]`）。**首次空记忆** 时脚本用 `--previous-limit`（默认 **3**）限制用于 diff 的历史条数，避免与大量旧记录误 diff。
...
- `--previous-limit` 默认为 `3`；`0` 表示用全部历史做 diff。
